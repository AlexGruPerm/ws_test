...
val getNextListFollowers: (List[ResponseCookie], Option[String]) => IO[NextFollowersCall] =
  (cookiesLogin, after) => for {
    //_ <- IO.sleep(sleepMs)
    reqInstFallWithHeadersNext <- IO{reqInstFollowers(FollowersProps(1356060226, 20, after), HeaderFollowers)}
...
val app1 :IO[Unit] = for {
  ...
  reqInstFallWithHeaders = reqInstFollowers(FollowersProps(1356060226,24,None),HeaderFollowers)
  ...
  _ <- IO{ println(s" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") }
  firstListFollowers = fFollowers.map(decoderFollower)

  nextFollowersCall1 <- getNextListFollowers(cookiesLogin,eCursor)
  listFollowersNext1 = nextFollowersCall1.listFollowers

  nextFollowersCall2 <- getNextListFollowers(cookiesLogin,Some(nextFollowersCall1.after))
  listFollowersNext2 = nextFollowersCall2.listFollowers

  nextFollowersCall3 <- getNextListFollowers(cookiesLogin,Some(nextFollowersCall2.after))
  listFollowersNext3 = nextFollowersCall3.listFollowers


  _ <- IO {
    (firstListFollowers ++ listFollowersNext1 ++ listFollowersNext2 ++ listFollowersNext3  ).zipWithIndex.foreach {
